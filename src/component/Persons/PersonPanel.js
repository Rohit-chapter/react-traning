...
let persons = [
  {id: '1',name: 'Vinita', age: 25},
  {id: '2',name: 'Rohit', age:28},
  {id: '3',name: 'Amitesh', age: 34},
  {id: '4',name: 'Ayushi', age: 60},
  {id: '5',name: 'Soahil', age: 28}
]
class PersonPanel extends Component {
  ...
  onSubmit = (event) => {
    ...
    if(this.state.person_name === '' || this.state.person_age === ''){
      ...
      const contArray = personAdd.length + 1;
        ...
        // });
      console.log('hello');
      axios.post('https://my-json-server.typicode.com/rohittaur/persondata/profile', {
        name: this.state.person_name,
        lastName: 'Flintstone'
      }).then(res => {
  ...
  render() {
    ...
    const person = _users.map((person,index) =>{
      return(
         <Person key={person.id} name={person.name} email={person.email} id={person.id} toggle={this.toggle} onDelete={this.handelDelete}/>
      )
    }
  );
